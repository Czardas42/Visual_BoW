...
visualize_confmat = 1;
visualize_res = 1;
have_screen = 1;% ~isempty(getenv('DISPLAY'));
...
% k-NN算法实现如下
    K = 1;
    rng(1);
    kNNClassifer = fitcknn(bof_train,labels_train, 'NumNeighbors', K);% , 'Distance', 'spearman');
%     kNNClassifer = fitcknn(bof_train,labels_train, 'NumNeighbors', K,'OptimizeHyperparameters','auto',...
%     'HyperparameterOptimizationOptions',...
%     struct('AcquisitionFunctionName','expected-improvement-plus'));
    bof_l2lab = predict(kNNClassifer,bof_test);
